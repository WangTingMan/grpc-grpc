...
using grpc::Status;
using helloworld::MessageFromServer;
using helloworld::MessageFromClient;
...
public:
  ...
  bool asycSendMsg( MessageFromClient& msg );
...
private:
  std::unique_ptr<helloworld::HelloService::Stub> stub;
  grpc::CompletionQueue* cq;//Completion Queue
  ClientContext clientContext;//每一个stream都有自己的clientContext
  std::unique_ptr<ClientAsyncReaderWriter<MessageFromClient, MessageFromServer>> stream;
  Status status;
  //函数指针
...
GrpcStreamClientInstance::GrpcStreamClientInstance( std::shared_ptr<Channel> channel, grpc::CompletionQueue* inputCq,
                                                    GrpcStreamClientThread* runThread ) :stub( helloworld::HelloService::NewStub( channel ) ), cq( inputCq ), threadInstance( runThread )
{
  ...
  stream = stub->AsyncBidirectionalStreamMessages( &clientContext, cq, &connectedFunc );
...
bool GrpcStreamClientInstance::asycSendMsg( MessageFromClient& msg )
{
    ...
    while( !isClose )
    {
      ...
      if( msgNum != 0 )
      {
        ...
        if( lock.try_lock_for( tryTime ) )
        {
          ...
          {
            MessageFromClient msg = msgQueue.front();
      ...
      if( isTimeElapsed( now, lastTime, 10000 * 1000 ) )
      {//判断是否已经过了10秒
        lastTime = now;
...
bool GrpcStreamClientThread::sendMsg( const  MessageFromClient& msg )
{
  {
    std::unique_lock<timed_mutex>lock( writeLock, std::defer_lock );
    chrono::milliseconds  tryTime( 500 );
...
int main()
{
  ...
  {
    msg.set_msg( "This is from client");
    msg.set_id( id++ );
